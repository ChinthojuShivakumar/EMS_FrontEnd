...
const AdminUsers = () => {
  ...
  const currentRows = roleList.slice(
  ...
  const deleteDepartment = async (_id) => {
    try {
      const response = await deleteApiV2(
        `/admin-role/delete/${_id}?adminId=${adminId}`
      );
      if (response.status == 200) {
        // clearInputs()
        fetchRoles();
              ...
              <table className="w-full">
                <thead className="bg-green-300">
                  <tr className="">
                    ...
                    <th className="py-2 border-r-2">Status</th>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-2 border-r-2">
                          {role?.menus?.map((menu) => (
                            <li className="text-start pl-5" key={menu?._id}>
                              {menu.title}
                            </li>
                          ))}
                        </td>
                        <td className="py-2 border-r-2 grid grid-cols-3">
                          {role?.subMenus?.map((menu) => (
                            <li className="text-start pl-5" key={menu?._id}>
                              {menu.title}
                            </li>
                          ))}
                          ...
                          <div className="w-full flex justify-center items-center gap-5">
                            {isEdit && (
                              <button
                                ...
                                onClick={(e) => handleEditUser(e, role?._id)}
                              >
                                Edit
                              </button>
                            )}
                            {isDelete && (
                              <button
                                ...
                                onClick={() => deleteDepartment(role?._id)}
                              >
                                Delete
                              </button>
                            )}
