...
import { fetchApiV2, postApiV2, putApiV2 } from "../../../Components/Constant/Network.V2"
...
const UserModal = () => {
    ...
    const fetchSubMenus = useCallback(async () => {
        ...
        try {
            const response = await fetchApiV2(`/submenu-permission?adminId=${adminId}`)
            if (response.status === 200) {
    ...
    const filterSubmenu = useCallback(() => {
        ...
        const array = submenuList.filter((submenu) => menuIds.includes(submenu.menuId._id)).map((submenu) => ({
            _id: submenu._id,
    ...
    const postAdminRole = async () => {
        try {
            const response = await postApiV2(`/admin-role/create?adminId=${adminId}`, inputs)
    ...
    const updateAdminRole = async () => {
        try {
            const response = await putApiV2(`/admin-role/update/${inputs._id}?adminId=${adminId}`, inputs)
                    ...
                    <div className="w-full">
                        ...
                        <div className="w-full mt-5 flex flex-col gap-5">
                            ...
                            <Autocomplete
                                ...
                                multiple
                                value={menuList.filter(item => inputs.menus.some((it => it._id === item._id)))}
                            ...
                            <Autocomplete
                                ...
                                fullWidth
                                sx={{ width: 600 }}
                                renderInput={(params) => <TextField {...params} label="SubMenu" />}
                                multiple
                                value={filteredSubMenuList.filter(item => inputs.subMenus.some((it => it._id === item._id)))}
