    ...
    <div className="w-full h-screen bg-white shadow-sm font-roboto ">
      <div className="w-full flex flex-col justify-start items-start py-2">
        <div className="w-full">
          {adminMenus
            .filter((menu) =>
              menusList.some((item) => item.title === menu.menu)
            )
            ?.map((menu, i) => {
              return (
                <NavLink
                ...
                >
                  {menu.icon}
                  <h1 className="max-sm:hidden sm:max-md:hidden md:max-lg:visible">
                    {menu.menu}
                  </h1>
                </NavLink>
              );
        ...
        <div
          className="w-full flex justify-start items-center gap-3 py-3 text-red-700 cursor-pointer px-2 hover:bg-green-700 hover:text-white hover:bg-opacity-85"
